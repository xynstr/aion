...
def write_vault(primary_provider: str, primary_key: str,
                extra_keys: dict, channels: dict) -> None:
    """Write all API keys and tokens encrypted into the credentials vault."""
    from plugins.credentials.credentials import _vault_set_field_sync

    # Primary provider key
    if primary_provider != "ollama" and primary_key:
        env_key = PROVIDERS[primary_provider]["key_env"]
        svc     = env_key.lower().replace("_api_key", "").replace("_key", "")
        _vault_set_field_sync(svc, env_key, primary_key)

    # Additional provider keys
    for env_var, api_key in extra_keys.items():
        if api_key:
            svc = env_var.lower().replace("_api_key", "").replace("_key", "")
            _vault_set_field_sync(svc, env_var, api_key)

    # Telegram
    if channels.get("TELEGRAM_BOT_TOKEN"):
        _vault_set_field_sync("telegram", "TELEGRAM_BOT_TOKEN", channels["TELEGRAM_BOT_TOKEN"])
    if channels.get("TELEGRAM_CHAT_ID"):
        _vault_set_field_sync("telegram", "TELEGRAM_CHAT_ID", channels["TELEGRAM_CHAT_ID"])

    # Discord
    if channels.get("DISCORD_BOT_TOKEN"):
        _vault_set_field_sync("discord", "DISCORD_BOT_TOKEN", channels["DISCORD_BOT_TOKEN"])

    # Slack
    if channels.get("SLACK_BOT_TOKEN"):
        _vault_set_field_sync("slack", "SLACK_BOT_TOKEN", channels["SLACK_BOT_TOKEN"])
    if channels.get("SLACK_APP_TOKEN"):
        _vault_set_field_sync("slack", "SLACK_APP_TOKEN", channels["SLACK_APP_TOKEN"])

    ok("Secrets written to encrypted vault (credentials/)")
...
def write_config(model: str, permissions: dict | None = None, advanced: dict | None = None) -> None:
    ...
    cfg["model"] = model
    if advanced:
        if advanced.get("port"):
            cfg["port"] = int(advanced["port"])
        if advanced.get("github_repo"):
            cfg["github_repo"] = advanced["github_repo"]
    if permissions:
...
def run_onboarding() -> None:
    try:
        ...
        write_vault(provider, api_key, extra_keys, channels)
